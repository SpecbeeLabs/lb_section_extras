...
namespace Drupal\lb_section_extras\EventSubscriber;
 ...
 */
class LbSectionFormAlter implements EventSubscriberInterface {
  ...
  /**
   * LbSectionFormAlter constructor.
   *
  ...
  public function alterForm(FormAlterEvent $event) {
    ...
    if ($form['#form_id'] == 'layout_builder_configure_section') {
      ...
      $styleSettingConfigs = $this->configFactory->get('lb_section_extras.settings');
   ...
   * {@inheritdoc}
   */
  protected function layoutBuilderSectionFormAlterValidate(array &$form, FormStateInterface $form_state) {
    ...
    if (isset($values['layout_settings']['section_attributes']['data'])) {
      ...
      foreach ($data_attrs as $data_attr) {
        ...
        if (substr($data_attr[0], 0, 5) !== 'data-') {
          $form_state->setError($form['layout_settings']['section_attributes']['data'], $this->t('Data attributes must begin with "data-"'));
        }
      }
    }
  }
   ...
   */
  protected function layoutBuilderAttributeValidation($value) {
